module Admin
  class DailyMovieRankingsController < BaseController
    ...
    def build_rank_differences(current_rankings, previous_rankings)
      ...
      # -----------------------------------------------
      # 　 当日のランキングを1件ずつ処理して、
      #    前日との順位差を求め、結果をハッシュにまとめる
      #
      #    each_with_object({}) は：
      #      - 繰り返しながらハッシュを作っていくメソッド
      #      - 「hash」は処理中ずっと同じハッシュを参照している
      # -----------------------------------------------
      current_rankings.each_with_object({}) do |ranking, hash|
        # 当日の映画に対応する「前日のランキング」を取得
        previous_ranking = previous_map[ranking.movie_id]

        # ------------------------------------------------------------
        #    前日のデータがある場合。順位の差を計算（前日順位 - 今日順位）。結果をhashに入れていく
        #    前日に存在しない場合（新規ランクイン）。nil を入れる。
        # ------------------------------------------------------------
